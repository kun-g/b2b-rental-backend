...
### 功能说明
1. 用户与商户账号在当前版本由平台方创建提供给使用者；
2. 平台方创建账号时除每个类型账号必填与可选填写信息以外，必须要选择一个账号类型；
...
| 录入物流+SN（TO_SHIP） | ❌ | ✅ | ✅ | ❌ | ❌ |
| 确认签收（SHIPPED） | ❌ | ✅ | ✅ | ❌ | ❌ |
| 超时补价（SHIPPED） | ✅ 支付超时的差价 | ✅ 填写超时时间 | ✅ 填写超时时间| ❌ | ❌ |
| 确认回寄（RETURNING） | ❌ | ✅ | ✅ | ❌ | ❌ |
| 确认完成（RETURNED） | ❌ | ✅ | ✅ | ❌ | ❌ |
| **运费模板** |
| 查看模板 | ❌ | ✅ | ✅ | ✅ | ✅ |
...
| PAID |已支付 | 用户一次性支付租金+运费；按 device_value 冻结授信 | 用户可取消 | **自动** → TO_SHIP | 去掉“商户接单”环节；已冻结授信以便后续扣款/补偿 |
...
| RETURNED | 已归还 | 回寄签收（商户确认已收到） | 商户可：1) 直接“确认完成”；2) 在**此时**选择“发起超时补价”（填写超时时段/补价金额）；用户可在补价发起后查看、支付 | 若商户**未**发起超时补价 → 商户确认完成 → COMPLETED；若商户发起超时补价 → LATE_PENDING | 关键点：商户只有在已归还（RETURNED）时发起补价；若发起则不会直接流转到 COMPLETED，必须走补价收款流程。 |
| LATE_PENDING | 超时补价·待支付 | 从 RETURNED（商户发起超时补价，订单价格更新，记录超时起止/补价明细） | 用户可支付超时补价  | 用户支付成功 → LATE_PAID； | 在此状态下**暂不释放授信**；显示“待用户支付超时补价”。  |
| LATE_PAID    | 超时补价·已支付 | 用户已完成超时补价支付  | 商户可确认完成 | 商户确认完成（人工确认） → COMPLETED； | 进入此状态时款项已到账；授信仍在 COMPLETED 时统一释放。 |
| COMPLETED | 已完成 | 商户确认完成（且若有超时补价，用户已支付）；或在无补价路径下商户直接确认完成  | —  | 生成对账单；释放授信；若有逾期，完成时支付（已处理） | 终态。注意：若存在未结清的逾期/补价，不能进入该状态。 |
